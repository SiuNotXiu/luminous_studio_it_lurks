...
public class flashlight_fov_wall_mask : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        //shape drawing
        camera_main = GameObject.Find("camera_main(dont_change_name)");
        origin = Vector3.zero;
    ...
    private void LateUpdate()
    {
        ...
        for (int i = 0; i <= ray_count; i++)
        {
            ...
            if (raycast_hit_2d.Length != 0)
            {
                ...
                {
                    if (raycast_hit_2d[j].collider.gameObject.name == "flashlight_trigger_area")
                    {
            ...
            /*Debug.Log("get_vector_from_angle(0) > " + get_vector_from_angle(0));
            Debug.Log("get_vector_from_angle(90) > " + get_vector_from_angle(90));
            Debug.Log("get_vector_from_angle(180) > " + get_vector_from_angle(180));
            Debug.Log("get_vector_from_angle(270) > " + get_vector_from_angle(270));*/
            vertex.z = origin.z;
            Debug.Log("vertex.z > " + vertex.z + " mask_z_local_position > " + mask_z_local_position);
